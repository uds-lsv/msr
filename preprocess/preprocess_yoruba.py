...
def preprocess():
    ...
    parser.add_argument('--data_root', type=str, default='/local/user_name_aabbcc/data/text_classification/Yoruba')
    parser.add_argument('--save_root', type=str, default='/local/user_name_aabbcc/data/wrench/tmp_data/Yoruba')
    ...
    # if label2int, it means that the original labels are already integers,
    # only need to map x to x-1.
    ...
    l2idx = {'world': 0, 'nigeria': 1, 'health': 2, 'sport': 3, 'politics': 4, 'africa': 5, 'entertainment': 6}
    idx2l = {v: k for k, v in l2idx.items()}

    l2idxYoruba = {'Gbogbo Agbaye': 0, 'Naijiria': 1, 'Ilera': 2, 'Ere idaraya': 3,
                   'Oṣelu': 4, 'Afirika': 5, 'Idanilaraya': 6}
    idx2lYoruba = {v: k for k, v in l2idxYoruba.items()}


    e2h = {'world': 'Gbogbo Agbaye', 'nigeria': 'Naijiria', 'health': 'Ilera', 'sport': 'Ere idaraya',
                'politics': 'Oṣelu', 'africa': 'Afirika', 'entertainment': 'Idanilaraya'}
    ...
    # After Label conversion, we can/should save the original labels in Yoruba.
    l2idx = l2idxYoruba
    idx2l = idx2lYoruba
...
def get_content_from_tsv(path):
    ...
    if len(df.columns) == 4:  # Yoruba data
        content = df[2].values.tolist()[1:]
        labels = df[3].values.tolist()[1:]
    elif len(df.columns) == 5:  # Yoruba noisy data
        content = df[3].values.tolist()[1:]
        labels = df[4].values.tolist()[1:]
    else:
        raise Exception(f"Unknown column number for Youruba dataset")
